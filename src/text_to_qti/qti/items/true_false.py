...
class TrueFalseGenerator(BaseItemGenerator):
    """Generate QTI XML for true/false questions."""

    def validate_question(self, question: Question) -> None:
        """Validate that question is a true/false question."""
        if question.type != QuestionType.TRUE_FALSE:
            raise ValueError(f"Expected TRUE_FALSE, got {question.type}")

    def generate(self, question: Question) -> etree._Element:
        """Generate true/false item XML.
        ...
        try:
            ...
            raise GenerationError(f"Failed to generate true/false item: {e}") from e
    ...
    def _add_metadata(self, item: etree._Element, question: Question) -> None:
        ...
        add_child(field, "fieldentry", "true_false_question")
